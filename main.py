...
from APNet import resnet_apnet as models
from APNet.loader import GaussianBlur, APNetTransform
from APNet.apnet import APNet
...
parser.add_argument('--min-lr', default=0.0, type=float,
                    metavar='LR', help='min learning rate for cos decay')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = APNet(models.__dict__[args.arch], args.proj_dim, args.loss_t)
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        APNetTransform(transforms.Compose(augmentation), args.cross_ratio))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    progress = ProgressMeter(
        args,
        len(train_loader),
        [batch_time, data_time, losses, losses_clu, losses_loc],
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        loss_clu, loss_loc = model(images, progress)
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    lr = args.lr
    lr *= (0.5-args.min_lr) * (1. + math.cos(math.pi * epoch / args.epochs)) + args.min_lr
