...
from JigClu.util import format_time
...
parser.add_argument('--lr-last-layer', default=30., type=float,
...
parser.add_argument('--prefix', default='module.encoder_q.', type=str,
                    help='prefix for sanity_check. ')
parser.add_argument('--cos', action='store_true',
                    help='use cosine lr schedule')
parser.add_argument('--syncbn', action='store_true',
                    help='use syncbn')

parser.add_argument('--labels-perc', default='', type=str,
                    help='labels percentage for semi supervised setting (1%, 10%)')
...
def save_checkpoint(args, state, is_best, filename='linear_checkpoint.pth.tar'):
    filename = os.path.join(args.save_dir, filename)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, os.path.join(args.save_dir, 'linear_model_best.pth.tar'))
...
def sanity_check(args, state_dict, pretrained_weights):
    ...
    """
    progress = ProgressMeter(args)
    progress.write("=> loading '{}' for sanity check".format(pretrained_weights))
    ...
    for k in list(state_dict.keys()):
        ...
        k_pre = args.prefix + k[len('module.'):] \
            if k.startswith('module.') else args.prefix + k

        assert ((state_dict[k].cpu() == state_dict_pre[k_pre]).all()), \
            '{} is changed in linear classifier training.'.format(k)

    progress.write("=> sanity check passed.")
...
class ProgressMeter(object):
    def __init__(self, args, num_batches=0, meters=None, prefix=""):
        ...
        self.meters = meters
        self.prefix = prefix
        self.file = open(os.path.join(args.save_dir, 'linear_log.txt'), 'a')
